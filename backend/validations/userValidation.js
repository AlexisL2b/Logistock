...
export const userSchema = Joi.object({
  ...
  nom: Joi.string().min(2).max(100).trim().required().messages({
    "string.min": "Le nom doit contenir au moins 2 caractères.",
    "string.max": "Le nom ne doit pas dépasser 100 caractères.",
    "any.required": "Le nom est obligatoire.",
  }),

  prenom: Joi.string().min(2).max(100).trim().required().messages({
    "string.min": "Le prénom doit contenir au moins 2 caractères.",
    "string.max": "Le prénom ne doit pas dépasser 100 caractères.",
    "any.required": "Le prénom est obligatoire.",
  }),

  adresse: Joi.string().min(5).max(255).trim().required().messages({
